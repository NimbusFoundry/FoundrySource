define([], ()->
	{
		...
		###
		  ...
		  the scope refers to a private component scope
		###
		onForumLoaded: ($scope, $foundry, $filter) ->
			...
			$scope.users = foundry._models.User.all().filter (x) -> x.name
			$scope.addTodo = (content) ->
				...
				msg = "#{foundry._current_user.name} added a new todo: <b>#{content}</b>"
			...
			$scope.delTodo = (todo) ->
				...
				msg = "#{foundry._current_user.name} deleted todo: <b>#{todo.content}</b>"
				$scope.$parent.log_comment(msg)
			...
			$scope.toggle = (todo)->
				...
				msg = "#{foundry._current_user.name} marked <b>'#{todo.content}'</b> #{status}"
			...
			$scope.assign = (todo, user) ->
				...
				email = foundry._user_list[todo["userid"]].email
				data = 
					subject : 'Forum Todo '
			...
			$scope.get_image_path = (id)->
				foundry._plugins.document._documents[id].webContentLink
